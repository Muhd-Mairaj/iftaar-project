...
const RESTAURANTS = [
  {
    ...
    description: "Authentic Kabsa & Mandi"
  },
  {
    id: 2,
    name: "Noor Kitchen",
    ...
    description: "Traditional Home-style Iftar"
  }
];
    ...
    <div className="flex-grow flex flex-col items-center p-6 transition-all duration-700 h-full absolute inset-0 pt-24 pb-28">
      <div className="w-full max-w-sm mx-auto flex flex-col h-full overflow-hidden">
        <header className="mb-6 px-2 text-center animate-in fade-in slide-in-from-top-4 duration-1000">
          <h1 className="text-2xl font-black text-foreground tracking-tight">Our Partners</h1>
          <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest mt-1 opacity-50">Verified Pricing & Details</p>
        </header>
        ...
        <div className="space-y-4 overflow-y-auto no-scrollbar pr-1 flex-grow pb-12">
          {RESTAURANTS.map((res) => (
            <div key={res.id} className="bg-card/40 backdrop-blur-xl border border-white/20 dark:border-white/5 rounded-[2.5rem] p-5 flex flex-col gap-4 group transition-all hover:bg-card/60 shadow-xl shadow-black/5 animate-in fade-in slide-in-from-bottom-4 duration-700">
              <div className="flex items-start justify-between">
                <div>
                  <h3 className="text-lg font-black text-foreground group-hover:text-primary transition-colors leading-none">{res.name}</h3>
                  <p className="text-[10px] text-muted-foreground font-medium leading-none mt-2 opacity-60">{res.description}</p>
                </div>
                <div className="text-right">
                  <div className="text-lg font-black text-primary tracking-tight">{res.price} <span className="text-[10px] font-medium opacity-60">SAR</span></div>
                  <div className="text-[8px] font-bold uppercase tracking-tighter opacity-40">Per Packet</div>
                </div>
              </div>
          ...
          <div className="p-6 rounded-[2.5rem] bg-lantern/5 border border-lantern/10 text-center space-y-3 mt-4 mb-20">
            ...
            <p className="text-[9px] font-bold text-lantern/80 uppercase tracking-widest leading-relaxed px-4">
              Kindly upload your receipt <br /> in the home tab after transfer
            </p>
          </div>
        </div>
      </div>
